...
public class BookSteps {
    ...
    public void setSearchParametersDate(final LocalDateTime from, final LocalDateTime to) {
        result = library.findBooks(from, to);
    }

    @When("the customer searches for a book with the title {string}")
    public void setSearchParametersSimilarTitle(String title) {
        result = library.findBooksByTitle(title);
    }

    @When("the customer searches for a book with the author {string}")
    public void setSearchParametersAuthor(String author) {
        result = library.findBooksByAuthor(author);
    }

    @Then("{int} books should have been found")
    public void verifyAmountOfBooksFound(final int booksFound) { assertThat(result.size(), equalTo(booksFound));}
